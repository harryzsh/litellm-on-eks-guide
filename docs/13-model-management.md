...
**现象：** 某些客户端（尤其经过第三方中转、或默认走 OpenAI/Qwen 风格的 SDK）发来的请求 400,报错类似:
...
1. **`enable_thinking`** 是 Qwen/DashScope 生态的字段,**不是** Anthropic/Bedrock 的标准参数。`drop_params: true` 只丢弃"它认识、但目标模型不支持"的标准 OpenAI 参数,对这种**不认识的野字段**会原样透传 → Bedrock 严格校验直接拒收。
...
```yaml
  ...
  - model_name: claude-opus-4-8
    litellm_params:
      ...
      aws_region_name: __BEDROCK_REGION__
      drop_params: true
      additional_drop_params: ["enable_thinking", "frequency_penalty", "presence_penalty", "temperature", "top_p"]
    # ...

  # opus-4-6 / sonnet-4-6 / haiku-4-5:支持 temperature,只 drop 野字段
  - model_name: claude-sonnet-4-6
    litellm_params:
      model: bedrock/global.anthropic.claude-sonnet-4-6
      aws_region_name: __BEDROCK_REGION__
      drop_params: true
      additional_drop_params: ["enable_thinking", "frequency_penalty", "presence_penalty"]
    # ...
```

> 注意:对支持 temperature 的模型(4-6/sonnet/haiku)**不要** drop `temperature`/`top_p`,否则会静默吞掉正常客户的采样设置。它们的 `temperature+top_p` 组合 400 属于客户端发了非法组合,应让客户端二选一,而非服务端兜底。`enable_thinking` 这类纯野字段才适合全系列 drop。
